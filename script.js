...
const childmenuItems = document.querySelectorAll(".submenu");
...
childmenuItems.forEach((item, index) => {
  item.addEventListener("click", () => {
    item.classList.toggle("show_childmenu");
    childmenuItems.forEach((item2, index2) => {
      if (index !== index2) {
        item2.classList.remove("show_childmenu");
      }
    });
  });
});
